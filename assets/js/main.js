...
(function ($) {
  ...
  function personite_navigation() {
    ...
    [].forEach.call(navbarDropdown, function (dropdown) {
      "mouseenter mouseleave click".split(" ").forEach(function (event) {
        dropdown.addEventListener(event, function (e) {
          if (
            window.innerWidth >= siteNavbar_expand &&
            siteNavbar_dropdownHover === true
          ) {
            // Hover
            ...
            if (event === "mouseenter") {
              if (_this.classList.contains("dropdown")) {
                ...
                if ($(_this).parent().find(".dropdown-target")) {
                  $(_this)
                    .parent()
                    .find(".dropdown-target.dropdown-submenu")
                    .removeClass("show dropdown-target");
                }
            ...
            if (
              ...
              e.target.parentNode.classList.contains("dropdown-submenu")
            ) {
              ...
              if (ddSubmenu.classList.contains("show")) {
                ...
                if (ddMenu.querySelectorAll(".dropdown-target").length > 0) {
                  var submenuNodeList = ddMenu.querySelectorAll(
                      ".dropdown-target.dropdown-submenu"
                    ),
                    i;

                  for (i = 0; i < submenuNodeList.length; ++i) {
                    submenuNodeList[i].classList.remove(
                      "show",
                      "dropdown-target"
                    );
                    submenuNodeList[i]
                ...
                if (
                  ddSubmenu.parentNode.querySelectorAll(".dropdown-target")
                    .length > 0
                ) {
                  var submenuNodeList = ddSubmenu.parentNode.querySelectorAll(
                      ".dropdown-target.dropdown-submenu"
                    ),
                    i;
    ...
    $(document).on("click touchstart", function (e) {
      if (
        $siteNavbar.is(e.target) ||
        $(e.target).closest(".site-navbar").hasClass("site-navbar") ||
        $(e.target).hasClass("navbar-toggler") ||
        $(e.target).hasClass("navbar-toggler-alternative")
      ) {
        return;
      }
  ...
  function personite_slider() {
    ...
    if ($slider.length > 0) {
      if (!$slider.hasClass("slick-initialized")) {
        ...
        $slider.on(
          "reInit swipe setPosition destroy afterChange",
